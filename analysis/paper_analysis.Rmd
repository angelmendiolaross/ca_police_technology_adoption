...
# adding institutional varibles 
ols <- lm(BWC ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini + pnhb + diff_pnhb + phis + diff_phis + log(mean_violent_crime) + log(mean_prop_crime) + log(pop_density) + log(sworn_officers) + civ_emp_ratio + log(police_infl_adj_per_cap), data = df)
summary(ols)

logit <- glm(BWC ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini + pnhb + diff_pnhb + phis + diff_phis + log(mean_violent_crime) + log(mean_prop_crime) + log(pop_density) + log(sworn_officers) + civ_emp_ratio + log(police_infl_adj_per_cap) + pnonmovers + gini, data = df, family = "binomial")
summary(logit)
# exponentiate coefficients to get odds ratios
tidy_output <- tidy(logit, exponentiate = TRUE)
# Print the tidy output
