...
```{r, include=FALSE}
# load the data
load("../data/aos_dvs.RData") # Atlas of Surveillance data
...
# number of cities with more than 2,500 people: 450
ivs_final %>% subset(year==2019 & total_pop>2500) %>% nrow()
...
# 22% of cities adopted body-worn-cameras, and 46% adopted automatic license plate reader systems. But just 16% of cities had adopted both.
df %>% subset(year==2018 & total_pop>=2500 & BWC == 1) # 103
df %>% subset(year==2018 & total_pop>=2500 & ALPR == 1) # 209
df %>% subset(year==2018 & total_pop>=2500 & ALPR == 1 & BWC == 1) # 76
df %>% subset(year==2018 & total_pop>=2500) %>% subset(ALPR == 1 | BWC == 1) # 236

# percent Republican quick info
df %>% subset(year==2018 & pvoteRepub>.5) # 89 Majority-Republican cities
...
# median population of cities with BWC?
df %>%
  filter(year == 2018, total_pop >= 2500, !is.na(sworn_officers)) %>%
  pull(sworn_officers) %>%
  median()

# majority Republican cities that adopted BWCs
df %>% subset(year==2018 & pvoteRepub>.5 & BWC==1)
```
...
# partisanship and controls
m1 <- lm(ALPR ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini, data = df)
summary(m1)

# with problem-driven account variables
m2 <- lm(ALPR ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini + pnhb + diff_pnhb + phis + diff_phis + log(mean_violent_crime) + log(mean_prop_crime), data = df)
summary(m2)

# adding institutional varibles 
ols <- lm(ALPR ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini + pnhb + diff_pnhb + phis + diff_phis + log(mean_violent_crime) + log(mean_prop_crime) + log(pop_density) + log(sworn_officers) + civ_emp_ratio + log(police_infl_adj_per_cap), data = df)
summary(ols)

logit <- glm(ALPR ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini + pnhb + diff_pnhb + phis + diff_phis + log(mean_violent_crime) + log(mean_prop_crime) + log(pop_density) + log(sworn_officers) + civ_emp_ratio + log(police_infl_adj_per_cap) + pnonmovers + gini, data = df, family = "binomial")
summary(logit)
...
# preparing to plot
# model 1: partisanship measure alongside our control variables
m1_df <-
    broom::tidy(m1) %>% filter(term %in% c("pvoteRepub")) %>% mutate(model = "Model 1")

# model 2: adds the problem-driven account variables: racial demographics and crime
m2_df <-
    broom::tidy(m2) %>% filter(term %in% c("pvoteRepub", "pnhb", "phis", "log(mean_violent_crime)", "log(mean_prop_crime)")) %>% mutate(model = "Model 2")

# model 3: adds the police agency variables, including population density, the number of sworn officers, the share of civilian employees, and police spending
ols_df <-
    broom::tidy(ols) %>% filter(term %in% c("pvoteRepub", "pnhb", "phis", "log(mean_violent_crime)", "log(mean_prop_crime)", "log(pop_density)", "log(sworn_officers)", "civ_emp_ratio", "log(police_infl_adj_per_cap)")) %>% mutate(model = "Model 3")
...
```{r}
# partisanship and controls
m1 <- lm(drones ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini, data = df)
summary(m1)

# with problem-driven account variables
m2 <- lm(drones ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini + pnhb + diff_pnhb + phis + diff_phis + log(mean_violent_crime) + log(mean_prop_crime), data = df)
summary(m2)

# adding institutional varibles 
ols <- lm(drones ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini + pnhb + diff_pnhb + phis + diff_phis + log(mean_violent_crime) + log(mean_prop_crime) + log(pop_density) + log(sworn_officers) + civ_emp_ratio + log(police_infl_adj_per_cap), data = df)
summary(ols)

logit <- glm(drones ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini + pnhb + diff_pnhb + phis + diff_phis + log(mean_violent_crime) + log(mean_prop_crime) + log(pop_density) + log(sworn_officers) + civ_emp_ratio + log(police_infl_adj_per_cap) + pnonmovers + gini, data = df, family = "binomial")
summary(logit)
...
# partisanship and controls
m1 <- lm(ring ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini, data = df)
summary(m1)

# with problem-driven account variables
m2 <- lm(ring ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini + pnhb + diff_pnhb + phis + diff_phis + log(mean_violent_crime) + log(mean_prop_crime), data = df)
summary(m2)

# adding institutional varibles 
ols <- lm(ring ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini + pnhb + diff_pnhb + phis + diff_phis + log(mean_violent_crime) + log(mean_prop_crime) + log(pop_density) + log(sworn_officers) + civ_emp_ratio + log(police_infl_adj_per_cap), data = df)
summary(ols)

logit <- glm(ring ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini + pnhb + diff_pnhb + phis + diff_phis + log(mean_violent_crime) + log(mean_prop_crime) + log(pop_density) + log(sworn_officers) + civ_emp_ratio + log(police_infl_adj_per_cap) + pnonmovers + gini, data = df, family = "binomial")
summary(logit)
# exponentiate coefficients to get odds ratios
tidy_output <- tidy(logit, exponentiate = TRUE)
# Print the tidy output
...
```{r, echo=FALSE}
# partisanship and controls
m1 <- lm(cam_registry ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini, data = df)
summary(m1)

# with problem-driven account variables
m2 <- lm(cam_registry ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini + pnhb + diff_pnhb + phis + diff_phis + log(mean_violent_crime) + log(mean_prop_crime), data = df)
summary(m2)

# adding institutional varibles 
ols <- lm(cam_registry ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini + pnhb + diff_pnhb + phis + diff_phis + log(mean_violent_crime) + log(mean_prop_crime) + log(pop_density) + log(sworn_officers) + civ_emp_ratio + log(police_infl_adj_per_cap), data = df)
summary(ols)

logit <- glm(cam_registry ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini + pnhb + diff_pnhb + phis + diff_phis + log(mean_violent_crime) + log(mean_prop_crime) + log(pop_density) + log(sworn_officers) + civ_emp_ratio + log(police_infl_adj_per_cap) + pnonmovers + gini, data = df, family = "binomial")
summary(logit)
# exponentiate coefficients to get odds ratios
tidy_output <- tidy(logit, exponentiate = TRUE)
# Print the tidy output
...
crime_map <- read_xlsx("../data/crime_mapping/crime_mapping_2020-11-15-005048.xlsx")
...
# partisanship and controls
m1 <- lm(crime_mapping ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini, data = df)
summary(m1)

# with problem-driven account variables
m2 <- lm(crime_mapping ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini + pnhb + diff_pnhb + phis + diff_phis + log(mean_violent_crime) + log(mean_prop_crime), data = df)
summary(m2)

# adding institutional varibles 
ols <- lm(crime_mapping ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini + pnhb + diff_pnhb + phis + diff_phis + log(mean_violent_crime) + log(mean_prop_crime) + log(pop_density) + log(sworn_officers) + civ_emp_ratio + log(police_infl_adj_per_cap), data = df)
summary(ols)

logit <- glm(crime_mapping ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini + pnhb + diff_pnhb + phis + diff_phis + log(mean_violent_crime) + log(mean_prop_crime) + log(pop_density) + log(sworn_officers) + civ_emp_ratio + log(police_infl_adj_per_cap) + pnonmovers + gini, data = df, family = "binomial")
summary(logit)
# exponentiate coefficients to get odds ratios
tidy_output <- tidy(logit, exponentiate = TRUE)
# Print the tidy output
...
nixle <- read_xlsx("../data/nixle/police_reformat.xlsx")
...
# partisanship and controls
m1 <- lm(nixle ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini, data = df)
summary(m1)

# with problem-driven account variables
m2 <- lm(nixle ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini + pnhb + diff_pnhb + phis + diff_phis + log(mean_violent_crime) + log(mean_prop_crime), data = df)
summary(m2)

# adding institutional varibles 
ols <- lm(nixle ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini + pnhb + diff_pnhb + phis + diff_phis + log(mean_violent_crime) + log(mean_prop_crime) + log(pop_density) + log(sworn_officers) + civ_emp_ratio + log(police_infl_adj_per_cap), data = df)
summary(ols)

logit <- glm(nixle ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini + pnhb + diff_pnhb + phis + diff_phis + log(mean_violent_crime) + log(mean_prop_crime) + log(pop_density) + log(sworn_officers) + civ_emp_ratio + log(police_infl_adj_per_cap) + pnonmovers + gini, data = df, family = "binomial")
summary(logit)
# exponentiate coefficients to get odds ratios
tidy_output <- tidy(logit, exponentiate = TRUE)
# Print the tidy output
...
```{r}
# partisanship and controls
m1 <- lm(BWC ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini, data = df)
summary(m1)

# with problem-driven account variables
m2 <- lm(BWC ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini + pnhb + diff_pnhb + phis + diff_phis + log(mean_violent_crime) + log(mean_prop_crime), data = df)
summary(m2)

# adding institutional varibles 
ols <- lm(BWC ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini + pnhb + diff_pnhb + phis + diff_phis + log(mean_violent_crime) + log(mean_prop_crime) + log(pop_density) + log(sworn_officers) + civ_emp_ratio + log(police_infl_adj_per_cap) + complaints_per_1000, data = df)
summary(ols)

logit <- glm(BWC ~ pvoteRepub + prenter + log(med_hh_inc_infl_adj) + pnonmovers + gini + pnhb + diff_pnhb + phis + diff_phis + log(mean_violent_crime) + log(mean_prop_crime) + log(pop_density) + log(sworn_officers) + civ_emp_ratio + log(police_infl_adj_per_cap) + pnonmovers + gini + complaints_per_1000, data = df, family = "binomial")
summary(logit)
# exponentiate coefficients to get odds ratios
tidy_output <- tidy(logit, exponentiate = TRUE)
# Print the tidy output
...
ivs <- ivs_final %>% filter(year==2018) %>%
  select(fips, City, total_pop, pvoteRepub, pnhb,diff_pnhb,phis, diff_phis, mean_violent_crime, mean_prop_crime, sworn_officers, civ_emp_ratio, police_infl_adj_per_cap, prenter, med_hh_inc_infl_adj, pnonmovers, pop_density, gini, ppov100)
